...
<Embed url="https://www.figma.com/file/0bGXR2sCwMVSDNyyzu5BXrO5/Desktop-UI?node-id=2000%3A1127" />


### Area 

<Embed url="https://www.figma.com/file/0bGXR2sCwMVSDNyyzu5BXrO5/Desktop-UI?node-id=2000%3A4942" />
...
### Label  
A label text is always required. This text helps users know what information is expected from this field. The label can either be above (as it is build into the component in Figma) or to the left of the field (done by hidding the built in label in Figma and adding a new label to the left).


### Meta  
Character count, required or optional labels can go in meta grouping. Do not assume that all users know what an "*" means, use *Required* as the meta. 


### Placeholder text   
Placeholder text should provide hints or examples of what text to enter. For wording, use "e.g. Stavanger" and not "Name your town". For numbers, use "9-digit number" and not "Example: 123-456-789". Placeholder text disappears after the user begins to type, so it should *not* contain critical information. In addition, not all screen readers are able to read placeholder text. If the user does not type within the field, the placeholder text reappears in the field. 
