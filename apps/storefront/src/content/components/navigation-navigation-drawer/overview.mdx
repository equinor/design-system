...
A navigation drawer is used to hold navigation with multiple groupings or many levels. Use a navigation drawer when the content will:
...
A `Child` can be used with and without a `Parent` and can be used with only a `Grandparent`. 
