...
Dividers are not meant to scream, they are subtle ways to group sections. Dividers can only be used in the colors `UI Background/Default`, `UI Background/Light` and `UI Background/Medium`. Dividers can be used full-width/full-bleed where allowed by parent container, or attached to a column's width. 
