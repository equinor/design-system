...
Selection controls are to be visible and understandable at a quick glance. Selected options are more viable than unselected. Selection controls are always accompanied with a clear label. Make sure that the labels are surrounded with a `Clickbound` that activates the selection control as well as the control itself.
