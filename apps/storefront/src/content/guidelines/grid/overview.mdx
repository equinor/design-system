...
Screen size is determined by the device and the user's window size. Our grid comes with three main content layouts that should cover all desktop, tablet and mobile needs adapting to different screen sizes and orientations.


### Grid layouts

* It is the responsibility of the designers to define a maximum width and choose the correct grid. 
* It is up to the designer to make sure that the design will scale correctly on all sizes. Start with a viewport that is the most common for the users. Make sure to test that the content will fit at smaller and larger viewports. 
...
Grid margins are the space that is at the outer edge of the grid against the frame's edge. 
