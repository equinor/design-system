...
Buttons in groups should always have the same color (`primary`, `secondary`, or `danger`), with an exception if one of the buttons are `disabled`. To emphasize and create a hierarchy you should use `contained` for high-emphasis actions, `outlined` for medium-emphasis, and `ghost` for low-emphasis. 
...
<Playground>
  {() => {
    const handleSubmit = (e) => {
      e.preventDefault() // to prevent navigation away
      alert('Submittet')
    }
    return (
