...
<DocsSection title="Install" tone="muted">
...
<DocsSection title="Use components">
...
New components live under the `/next` export, published on the `beta` dist-tag. They use vanilla CSS with design tokens instead of styled-components:
...
:::note
`/next` ships only on the `beta` dist-tag, so the plain `npm install @equinor/eds-core-react` above does not include it. Browsing the components in Storybook does not enable them either.

Beta here means safe to adopt alongside EDS 1.0, not "wait and see": EDS 1.0 stays supported until EDS 2.0 becomes stable, and mixing both in one application is expected. The API may still change in small ways, and the token setup for `/next` is being reworked. See [About EDS 2.0](https://storybook.eds.equinor.com/?path=/docs/eds-2-0-beta-about--docs) for what beta means.
:::

### Versioning

"EDS 2.0" is the name of the next generation of the design system. Its components currently live in beta under the `/next` entry point of the `eds-core-react` package, published on the `beta` dist-tag rather than in the default `latest` install. The package itself is on version `2.x` today, so an installed `eds-core-react@2.x` matching "EDS 2.0" is also a coincidence.
...
<DocsSection title="Design tokens" tone="muted">
