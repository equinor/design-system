...
[Getting Started](./getting_started.mdx#reading-a-token-name) explains how to read a name, and it is
...
[Usage](./usage.mdx#start-with-behaviour) covers the distinction.
...
instead of translating by strength alone. [Interactive borders](./usage.mdx#border) have only
...
Use APCA to assess text and controls against their actual backgrounds in both schemes and all
applicable states, with the intended text size and weight. Do not treat a default-state pairing as
proof that its hover and pressed states also pass. See [Contrast](./intro.mdx#contrast)
and [Foreground pairing](./usage.mdx#foreground-pairing).
