...
Each icon has needs to be reduced to its minimal form - being bold, geometrical and symetrical. To reflect the rounded corners of the logo, icons neeed to have rounded corners of 2 pixels where possible - however, internal corners should be square, not rounded. To reflect the brand, icons use a consistent stroke of 2 pixels - this inculdes curves, angles and both interior and exterior strokes. The defualt icon is outline, a filled icon may be provided only if there is an outlined icon.
...
* Create an artboard that is 24 x 24 pixels. 
  * Place the `.System icon grid`from the Utilites page in Assets File as the bottom layer and lock it. 
