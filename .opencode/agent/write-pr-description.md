...
1. Read the diff against the base branch (`git diff origin/main...HEAD`), the commit log (`git log origin/main..HEAD`) and the linked issue. If the user names a PR number, read that PR with `gh pr view` instead.
