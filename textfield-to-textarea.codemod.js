...
module.exports = function transformer(file, api) {
  ...
  root
    ...
    .filter(path => {
      ...
      if (elementName.type === 'JSXIdentifier' && elementName.name === 'TextField') {
  ...
  if (hasChanges) {
    ...
    root
      ...
      .forEach(path => {
        ...
        const hasTextarea = specifiers.some(spec => 
          spec.type === 'ImportSpecifier' && 
        ...
        if (!hasTextFieldSingleLine) {
          path.value.specifiers = specifiers.filter(spec => 
            !(spec.type === 'ImportSpecifier' && spec.imported.name === 'TextField')
          );
