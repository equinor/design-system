...
| ------------------------------------------------- | --------------------------------------------------- | ------------------------------------------ |
| `NPM_TOKEN`                                       | All publish workflows                               | npm registry authentication                |
...
- **Cache miss:** The setup job caches using `github.sha`. If the cache was evicted, the setup re-runs. Check the Actions cache tab.
...
- **npm auth error:** Verify `NPM_TOKEN` is valid and not expired in repository secrets.
