...
Run the weekly Dependabot rotation: triage the open Dependabot PRs **and** the open alerts on the Security tab, then produce a report with recommendations and the exact `gh` commands to run. Scope can be narrowed with **$ARGUMENTS** (`prs`, `alerts`, a PR number, or a package name); with no arguments, do both halves.
