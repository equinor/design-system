...
async function b({ tokensDir: e, cssBuildPath: t, cssTransforms: n }) {
	...
	let C = await f.promises.readdir(x).catch(() => []), w = (e) => e === "size-extras.ts" || e.startsWith("font-family-");
	...
	let ge = [
		...
		Object.entries(Q).map(([e, t]) => `  '${e}': { iconSize: ${t.iconSize}, gapHorizontal: ${t.gapHorizontal}, gapVertical: ${t.gapVertical} },`).join("\n"),
		"} as const",
		""
	].join("\n");
	await f.promises.writeFile(u.join(x, "size-extras.ts"), ge);
	for (let e of X) {
		let t = e.toLowerCase();
