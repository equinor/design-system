/**
 * Do not edit directly, this file was auto-generated.
 */

export const spacing = {
  sizing: {
    icon: {
      xs: 16,
      sm: 18,
      md: 20,
      lg: 24,
      xl: 28,
      twoXl: 32,
      threeXl: 37,
      fourXl: 42,
      fiveXl: 48,
      sixXl: 56,
    },
    selectable: {
      xs: 20,
      sm: 24,
      md: 36,
      lg: 44,
      xl: 52,
      twoXl: 60,
    },
    stroke: {
      thin: 1,
      thick: 2,
      none: 0,
    },
  },
  spacing: {
    icon: {
      xs: {
        gapHorizontal: 6.5,
        gapVertical: 6.5,
      },
      sm: {
        gapHorizontal: 7.5,
        gapVertical: 7.5,
      },
      md: {
        gapHorizontal: 8.5,
        gapVertical: 8.5,
      },
      lg: {
        gapHorizontal: 10,
        gapVertical: 10,
      },
      xl: {
        gapHorizontal: 11.5,
        gapVertical: 11.5,
      },
      twoXl: {
        gapHorizontal: 13,
        gapVertical: 13,
      },
      threeXl: {
        gapHorizontal: 15,
        gapVertical: 15,
      },
      fourXl: {
        gapHorizontal: 17.5,
        gapVertical: 17.5,
      },
      fiveXl: {
        gapHorizontal: 20,
        gapVertical: 20,
      },
      sixXl: {
        gapHorizontal: 23,
        gapVertical: 23,
      },
    },
    borderRadius: {
      rounded: 4,
      pill: 1000,
      none: 0,
    },
    inset: {
      xs: {
        horizontal: 8,
        verticalSquished: 6,
        verticalSquared: 8,
        verticalStretched: 12,
      },
      sm: {
        horizontal: 12,
        verticalSquished: 8,
        verticalSquared: 12,
        verticalStretched: 16,
      },
      md: {
        horizontal: 16,
        verticalSquished: 12,
        verticalSquared: 16,
        verticalStretched: 20,
      },
      lg: {
        horizontal: 20,
        verticalSquished: 16,
        verticalSquared: 20,
        verticalStretched: 24,
      },
      xl: {
        horizontal: 24,
        verticalSquished: 20,
        verticalSquared: 24,
        verticalStretched: 28,
      },
    },
    horizontal: {
      fourXs: 2,
      threeXs: 4,
      twoXs: 6,
      xs: 8,
      sm: 12,
      md: 16,
      lg: 20,
      xl: 24,
      twoXl: 28,
      threeXl: 32,
    },
    vertical: {
      fourXs: 2,
      threeXs: 4,
      twoXs: 6,
      xs: 8,
      sm: 12,
      md: 16,
      lg: 20,
      xl: 24,
      twoXl: 28,
      threeXl: 32,
    },
  },
  typography: {
    uiBody: {
      xs: {
        fontSize: 10.5,
        trackingTight: -1.149999976158142,
        trackingNormal: 0,
        trackingWide: 1.149999976158142,
        fontWeightLighter: 300,
        fontWeightNormal: 400,
        fontWeightBolder: 500,
        lineHeightDefault: 16,
        lineHeightSquished: 12,
      },
      sm: {
        fontSize: 12,
        trackingTight: -1.3200000524520874,
        trackingNormal: 0,
        trackingWide: 1.3200000524520874,
        fontWeightLighter: 300,
        fontWeightNormal: 400,
        fontWeightBolder: 500,
        lineHeightDefault: 16,
        lineHeightSquished: 12,
      },
      fontFamily: 'Inter',
      md: {
        fontSize: 14,
        trackingTight: -1.5399999618530273,
        trackingNormal: 0,
        trackingWide: 1.5399999618530273,
        fontWeightLighter: 300,
        fontWeightNormal: 400,
        fontWeightBolder: 500,
        lineHeightDefault: 20,
        lineHeightSquished: 16,
      },
      lg: {
        fontSize: 16,
        trackingTight: -1.7599999904632568,
        trackingNormal: 0,
        trackingWide: 1.7599999904632568,
        fontWeightLighter: 300,
        fontWeightNormal: 400,
        fontWeightBolder: 500,
        lineHeightDefault: 24,
        lineHeightSquished: 20,
      },
      xl: {
        fontSize: 18.5,
        trackingTight: -1.7599999904632568,
        trackingNormal: 0,
        trackingWide: 2.0350000858306885,
        fontWeightLighter: 300,
        fontWeightNormal: 400,
        fontWeightBolder: 500,
        lineHeightDefault: 24,
        lineHeightSquished: 20,
      },
      twoXl: {
        fontSize: 21,
        trackingTight: -2.309999942779541,
        trackingNormal: 0,
        trackingWide: 2.309999942779541,
        fontWeightLighter: 300,
        fontWeightNormal: 400,
        fontWeightBolder: 500,
        lineHeightDefault: 28,
        lineHeightSquished: 24,
      },
      threeXl: {
        fontSize: 24.5,
        trackingTight: -2.694999933242798,
        trackingNormal: 0,
        trackingWide: 2.694999933242798,
        fontWeightLighter: 300,
        fontWeightNormal: 400,
        fontWeightBolder: 500,
        lineHeightDefault: 32,
        lineHeightSquished: 28,
      },
      fourXl: {
        fontSize: 28,
        trackingTight: -3.0799999237060547,
        trackingNormal: 0,
        trackingWide: 3.0799999237060547,
        fontWeightLighter: 300,
        fontWeightNormal: 400,
        fontWeightBolder: 500,
        lineHeightDefault: 32,
        lineHeightSquished: 28,
      },
      fiveXl: {
        fontSize: 32,
        trackingTight: -3.5199999809265137,
        trackingNormal: 0,
        trackingWide: 3.5199999809265137,
        fontWeightLighter: 300,
        fontWeightNormal: 400,
        fontWeightBolder: 500,
        lineHeightDefault: 36,
        lineHeightSquished: 32,
      },
      sixXl: {
        fontSize: 37,
        trackingTight: -4.070000171661377,
        trackingNormal: 0,
        trackingWide: 4.070000171661377,
        fontWeightLighter: 300,
        fontWeightNormal: 400,
        fontWeightBolder: 500,
        lineHeightDefault: 40,
        lineHeightSquished: 36,
      },
    },
    header: {
      fontFamily: 'Equinor',
      xs: {
        fontSize: 12,
        trackingTight: -1.149999976158142,
        trackingNormal: 0,
        trackingWide: 1.149999976158142,
        fontWeightLighter: 300,
        fontWeightNormal: 400,
        fontWeightBolder: 500,
        lineHeightDefault: 16,
        lineHeightSquished: 12,
      },
      sm: {
        fontSize: 14,
        trackingTight: -1.3200000524520874,
        trackingNormal: 0,
        trackingWide: 1.3200000524520874,
        fontWeightLighter: 300,
        fontWeightNormal: 400,
        fontWeightBolder: 500,
        lineHeightDefault: 16,
        lineHeightSquished: 12,
      },
      md: {
        fontSize: 16,
        trackingTight: -1.5399999618530273,
        trackingNormal: 0,
        trackingWide: 1.5399999618530273,
        fontWeightLighter: 300,
        fontWeightNormal: 400,
        fontWeightBolder: 500,
        lineHeightDefault: 20,
        lineHeightSquished: 16,
      },
      lg: {
        fontSize: 18,
        trackingTight: -1.7599999904632568,
        trackingNormal: 0,
        trackingWide: 1.7599999904632568,
        fontWeightLighter: 300,
        fontWeightNormal: 400,
        fontWeightBolder: 500,
        lineHeightDefault: 24,
        lineHeightSquished: 20,
      },
      xl: {
        fontSize: 21,
        trackingTight: -1.7599999904632568,
        trackingNormal: 0,
        trackingWide: 2.0350000858306885,
        fontWeightLighter: 300,
        fontWeightNormal: 400,
        fontWeightBolder: 500,
        lineHeightDefault: 24,
        lineHeightSquished: 20,
      },
      twoXl: {
        fontSize: 24,
        trackingTight: -2.309999942779541,
        trackingNormal: 0,
        trackingWide: 2.309999942779541,
        fontWeightLighter: 300,
        fontWeightNormal: 400,
        fontWeightBolder: 500,
        lineHeightDefault: 28,
        lineHeightSquished: 24,
      },
      threeXl: {
        fontSize: 27.5,
        trackingTight: -2.694999933242798,
        trackingNormal: 0,
        trackingWide: 2.694999933242798,
        fontWeightLighter: 300,
        fontWeightNormal: 400,
        fontWeightBolder: 500,
        lineHeightDefault: 32,
        lineHeightSquished: 28,
      },
      fourXl: {
        fontSize: 31.5,
        trackingTight: -3.0799999237060547,
        trackingNormal: 0,
        trackingWide: 3.0799999237060547,
        fontWeightLighter: 300,
        fontWeightNormal: 400,
        fontWeightBolder: 500,
        lineHeightDefault: 32,
        lineHeightSquished: 28,
      },
      fiveXl: {
        fontSize: 36.5,
        trackingTight: -3.5199999809265137,
        trackingNormal: 0,
        trackingWide: 3.5199999809265137,
        fontWeightLighter: 300,
        fontWeightNormal: 400,
        fontWeightBolder: 500,
        lineHeightDefault: 36,
        lineHeightSquished: 32,
      },
      sixXl: {
        fontSize: 42,
        trackingTight: -4.070000171661377,
        trackingNormal: 0,
        trackingWide: 4.070000171661377,
        fontWeightLighter: 300,
        fontWeightNormal: 400,
        fontWeightBolder: 500,
        lineHeightDefault: 40,
        lineHeightSquished: 36,
      },
    },
  },
  generic: {
    gap: {
      none: {
        horizontal: 0,
        vertical: 0,
      },
      fourXs: {
        horizontal: 2,
        vertical: 2,
      },
      threeXs: {
        horizontal: 4,
        vertical: 4,
      },
      twoXs: {
        horizontal: 6,
        vertical: 6,
      },
      xs: {
        horizontal: 8,
        vertical: 8,
      },
      sm: {
        horizontal: 12,
        vertical: 12,
      },
      md: {
        horizontal: 16,
        vertical: 16,
      },
      lg: {
        horizontal: 20,
        vertical: 20,
      },
      xl: {
        horizontal: 24,
        vertical: 24,
      },
      twoXl: {
        horizontal: 28,
        vertical: 28,
      },
      threeXl: {
        horizontal: 32,
        vertical: 32,
      },
    },
    space: {
      none: {
        horizontal: 0,
        vertical: 0,
      },
      fourXs: {
        horizontal: 2,
        vertical: 2,
      },
      threeXs: {
        horizontal: 4,
        vertical: 4,
      },
      twoXs: {
        horizontal: 6,
        vertical: 6,
      },
      xs: {
        horizontal: 8,
        vertical: 8,
      },
      sm: {
        horizontal: 12,
        vertical: 12,
      },
      md: {
        horizontal: 16,
        vertical: 16,
      },
      lg: {
        horizontal: 20,
        vertical: 20,
      },
      xl: {
        horizontal: 24,
        vertical: 24,
      },
      twoXl: {
        horizontal: 28,
        vertical: 28,
      },
      threeXl: {
        horizontal: 32,
        vertical: 32,
      },
    },
  },
  spacingProportions: {
    squished: {
      xs: {
        horizontal: 8,
        vertical: 6,
      },
      sm: {
        horizontal: 12,
        vertical: 8,
      },
      md: {
        horizontal: 16,
        vertical: 12,
      },
      lg: {
        horizontal: 20,
        vertical: 16,
      },
      xl: {
        horizontal: 24,
        vertical: 20,
      },
    },
    squared: {
      xs: {
        horizontal: 8,
        vertical: 8,
      },
      sm: {
        horizontal: 12,
        vertical: 12,
      },
      md: {
        horizontal: 16,
        vertical: 16,
      },
      lg: {
        horizontal: 20,
        vertical: 20,
      },
      xl: {
        horizontal: 24,
        vertical: 24,
      },
    },
    stretched: {
      xs: {
        horizontal: 8,
        vertical: 12,
      },
      sm: {
        horizontal: 12,
        vertical: 16,
      },
      md: {
        horizontal: 16,
        vertical: 20,
      },
      lg: {
        horizontal: 20,
        vertical: 24,
      },
      xl: {
        horizontal: 24,
        vertical: 28,
      },
    },
  },
  selectableSpace: {
    xs: {
      horizontal: 8,
      vertical: 8,
    },
    sm: {
      horizontal: 12,
      vertical: 12,
    },
    md: {
      horizontal: 16,
      vertical: 16,
    },
    lg: {
      horizontal: 20,
      vertical: 20,
    },
    xl: {
      horizontal: 24,
      vertical: 24,
    },
  },
  containerSpace: {
    horizontal: 16,
    vertical: 16,
  },
  pageSpace: {
    horizontal: 24,
    vertical: 24,
  },
  genericGapHorizontal: 8,
  genericGapVertical: 8,
  genericSpaceHorizontal: 8,
  genericSpaceVertical: 8,
  selectableSpaceHorizontal: 8,
  selectableSpaceVertical: 8,
  spacingProportionsXsHorizontal: 8,
  spacingProportionsXsVertical: 8,
  spacingProportionsSmHorizontal: 12,
  spacingProportionsSmVertical: 12,
  spacingProportionsMdHorizontal: 16,
  spacingProportionsMdVertical: 16,
  spacingProportionsLgHorizontal: 20,
  spacingProportionsLgVertical: 20,
  spacingProportionsXlHorizontal: 24,
  spacingProportionsXlVertical: 24,
  selectableGapHorizontal: 8,
  selectableGapVertical: 8,
  containerGapHorizontal: 16,
  containerGapVertical: 16,
  pageGapHorizontal: 24,
  pageGapVertical: 24,
} as const
