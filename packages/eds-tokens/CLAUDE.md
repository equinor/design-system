...
The **legacy pipeline** below (Figma REST sync + Style Dictionary) still owns everything the *stable* package publishes (`build/`, the `exports` map). The **new Tokens Studio pipeline** writes generated output to `src/tokens/{<alias>,css,dtcg,ts}` via `.github/workflows/tokens_studio_release.yaml` — those directories are **generated, never edit them by hand**. That output is published only on the beta line: `src/tokens` is the root of the `eds-tokens-next` release-please component (pinned `3.0.0-beta.N` series on the npm `beta` dist-tag), and `publish_tokens.yaml` injects the generated directories into `files`/`exports` at beta publish time. `src/tokens/version.txt` and `src/tokens/CHANGELOG.md` are release-please-owned, not pipeline-generated. The TS modules are produced by `pnpm run generate:ts-tokens` (`scripts/generate-ts-tokens.mjs` — DTCG export for structure + CSS export for evaluated values; the script header documents the mechanics). Canonical pipeline doc: [`documentation/agent-instructions/TOKENS_STUDIO.md`](../../documentation/agent-instructions/TOKENS_STUDIO.md).
