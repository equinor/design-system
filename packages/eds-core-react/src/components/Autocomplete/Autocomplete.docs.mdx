...
If you instead want to control your own state, you can use the `selectedOptions` prop to pass your controlled data and `onOptionsChange` to 
handle changes in selection. The initial value can be an empty array if no elements are to be pre-selected. 
...
<ul>
  <li>The property <code>label</code> is mandatory and make sure you have a descriptive label for screen readers.</li>
  <li>If <code>clearSearchOnChange</code> is set to <code>false</code> make sure <code>label</code> has description of how many options are selected for screen readers.</li>
</ul>
