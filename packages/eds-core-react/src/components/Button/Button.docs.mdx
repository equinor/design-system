...
#### ARIA
<ul>
  <li>Button.Group has <code>role="group"</code>. The word <code>group</code> will be red by assistive technologies.</li>
...
Exclusive selection is a default sate, which means that by, selecting one option deselects any other.

<Story id="inputs-button--toggle-exclusive-selection" />

### Multiple selection

Multiple selection allows for logically-grouped options, to have multiple options selected, by using the `multiple` prop.

<Story id="inputs-button--toggle-multiple" />

### Controlled

To enforce at least one button to be active, you can adapt your `handleChange` function with `selectedIndexes` prop.

<Story id="inputs-button--toggle-controlled" />
