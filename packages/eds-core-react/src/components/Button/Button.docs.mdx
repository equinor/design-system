...
<ul>
  ...
  <li>controls the selected state of its child buttons when given its own `selected` prop.</li>
</ul>

```tsx
import { Button } from '@equinor/eds-core-react'
import { save, edit, copy } from '@equinor/eds-icons'

<Button.Toggle aria-label="toggle button group">
...
### Toggle Button
<Story id="inputs-button--toggle-button" />