...
<ul>
  ...
  <li>controls the selected state of its child buttons when given its own <code>selected</code> prop.</li>
</ul>
...
<Button.Toggle aria-label="actions" selected={selectedButton} onChange={handleChange}>
...
To enforce at least one button to be active, you can adapt your `handleChange` function.

<Story id="inputs-button--toggle-enforced" />

### Vertical

The `Button.Toggle` can be displayed vertically using the `vertical` prop.

<Story id="inputs-button--toggle-vertical" />
