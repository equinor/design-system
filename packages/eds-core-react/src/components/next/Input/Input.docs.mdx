...
Use `leftText` and `rightText` for prefix/suffix text (e.g., "$", "USD", "km"). These stay neutral color in all states.

Use `leftAdornment` and `rightAdornment` for elements (icons, buttons, etc.) that should inherit state colors (e.g., red when invalid).
