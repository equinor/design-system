...
In compact UIs like tables or toolbars, you may need a switch without a visible label. Use `aria-label` to provide an accessible name for screen readers.
...
- **With label:** Accessible name is provided automatically
- **Without label:** You must provide `aria-label` or `aria-labelledby`
