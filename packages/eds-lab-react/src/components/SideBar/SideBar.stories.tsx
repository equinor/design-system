...
export const WithCustomFooter: Story = () => {
  ...
  const Footer = () => {
    ...
    return <>{isOpen ? <LogoLarge /> : <Logo />}</>
  }

  return (
    <div style={{ display: 'flex', height: '95vh' }}>
      <SideBar toggleButton="bottom" footer={<Footer />}>
        {menuItems.map((m) => (
          <SideBar.Item key={m.name} {...m} />
        ))}
      </SideBar>
...
WithCustomFooter.parameters = {
  docs: {
    description: {
      story: `The footer param takes custom html to add logo/copyright notice/link etc at the bottom of the sidebar. Use <code>useSideBar</code> to correctly detect sidebar state in your custom footer if you need to modify content based on open/closed state.`,
    },
  },
}
