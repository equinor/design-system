...
function generateSemantic(
  ...
  // The semantic structure here mirrors the user's existing Semantic.Mode 1.json shape:
  // - Bg: Category -> Canvas/Surface/Fill Muted (Default/Hover/Active)/Fill Emphasis (Default/Hover/Active)
  // - Border: Category -> Subtle/Medium/Strong
  // - Text: Category -> Subtle/Strong/Subtle on emphasis/Strong on emphasis
  // Numeric indices from palettes are fixed: Bg uses 1..5 and 9..11 paths, Border uses 6..8, Text uses 12..15
  ...
  for (const [semantic, palette] of Object.entries(mapping)) {
    const intent = slug(semantic); // e.g. "Neutral" -> "neutral"
    // BG names
