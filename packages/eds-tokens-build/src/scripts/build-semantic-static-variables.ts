...
async function combineColorsFromConfig(cfg: TokenConfig) {
  ...
  // Remove self-referencing concept token declarations.
  // Concept tokens (bg-floating, bg-disabled, etc.) are included in the Style Dictionary
  // build so they appear in TS/JS/JSON exports, but with outputReferences: true they
  // produce self-references in CSS (e.g. --eds-color-bg-floating: var(--eds-color-bg-floating)).
  // These variables are already correctly defined with light-dark() values in color-scheme.css,
  // so we strip the self-references to avoid overwriting them during CSS bundling.
  const filteredInner = semanticInner
    .split('\n')
    .filter((line) => {
      const match = line.match(/--([\w-]+):\s*var\(--([\w-]+)\)/)
  ...
  const mergedRoot = [':root {', indentLines(filteredInner), '}'].join('\n')
