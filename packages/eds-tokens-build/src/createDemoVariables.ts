...
export async function createDemoVariables({
  tokensDir,
  cssTransforms,
}: {
  tokensDir: string
  ...
  const PADDING_XS_SOURCE = path.join(tokensDir, FILE_KEY, '03 Padding.XS.json')
  const PADDING_SM_SOURCE = path.join(tokensDir, FILE_KEY, '03 Padding.SM.json')
  const PADDING_MD_SOURCE = path.join(tokensDir, FILE_KEY, '03 Padding.MD.json')
